...
var Player = function(){
	...
	this.addInterface = function(interf){
	...
	// why use the interfaces on track and not on the player?
	this.noteOn = function(note, track){
		if(!note.isoff){
			this.noteOff(note, track)
		}
		var ii = (track.interfaces.length>0) ? track.interfaces : this.interfaces;
	...
	this.add = function(track, i){
		this.tracks.push(track);
		track.player = this;
		//track.inter = ( !track.driver )  ?  this.interfaces  :  track.driver;
		this.updateDuration()
		this.changed();
	}
